...
(function () {
  ...
  function searchEvent() {
    ...
    const event = Store.getEvents().find(e => e.codigo === code);
  ...
  function renderStudentResult(student, event, attendance) {
    ...
    if (!attendance) { info.style.display = 'none'; return; }

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set('student-name',       `${student.nombre}`);
    set('student-id',         student.id);
    set('student-career',     student.carrera);
    set('event-name',         event?.nombre || '');
    set('location-coords',    `${attendance.location.lat.toFixed(6)}, ${attendance.location.lng.toFixed(6)}`);
    set('location-precision', `${attendance.location.accuracy}m (${attendance.location.distance}m del evento)`);
    set('attendance-time',    new Date(attendance.timestamp).toLocaleString('es-CO'));
